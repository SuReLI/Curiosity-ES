...
for k in range(5):
	...
	algo=Map_elites( env=env, lamb=56, N=1000, sigma=0.005,map_size=50)
	algo.train()
	# shutdown
	ray.shutdown()
	wandb.finish()
	...
	# ray init
	ray.init() if slurm=='false' else ray.init(address='auto')
	algo=Map_elites( env=env, lamb=56, N=2000, sigma=0.005,map_size=50)
	algo.train()
	# shutdown
	ray.shutdown()
	wandb.finish()
	...
	# ray init
	ray.init() if slurm=='false' else ray.init(address='auto')
	algo=Map_elites( env=env, lamb=56, N=2000, sigma=0.001,map_size=50)
	algo.train()
	# shutdown
	ray.shutdown()
	wandb.finish()
