...
class CMA_me(Algorithm):

	def __init__(self, env, lamb=56, N=2000, sigma=0.001,map_size=50) :
		super().__init__(env, lamb, algo_name='cma_me_new_2', nb_epoch=N, map_size=map_size)
		# archive 
		...
		# emitter
		self.emitters=[ImprovementEmitter(self.archive, x0=np.zeros(shape=ray.get(self.population[0].genome.remote()).shape), sigma0= sigma, batch_size=lamb )]
		# optimizer
