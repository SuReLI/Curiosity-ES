...
class ICM(tf.keras.Model): 
	...
	def __init__(self,env, alpha_icm=1e-4, p= 5, m=10, beta=0.2, batch_size=2048, max_buffer=10000, gamma=0.99, dim_enc=32, k=0):
		...
		self.k=1
		### LAYERS ###
		self.LR1=Dense(32,"relu",name="LR1")
		self.LR2=Dense(32,"relu",name="LR2")
		self.LR3=Dense(dim_enc,"relu",name="LR3") 
		...
		self.F2=Dense(32,"sigmoid",name="Forward_Model3")
		self.F3=Dense(dim_enc,"relu",name="Forward_Model3") 
		...
		for k in range(self.p):
			...
			with tf.device('/CPU:0'):
				with tf.GradientTape() as tape:
					...
					Lreg=self.loss(Xn,X)
					# ******L*******
					Loss=self.beta*LF+(1-self.beta)*LI+Lreg
	...
	def get_curiosity(self,states,actions):
		...
		X=self.reg(S1)
		Xn=self.regn(S1)
		Lreg=self.loss(Xn,X,sample_weight=self.gamma**np.arange(X.shape[0],0,-1))
		# LI
		x=tf.concat([PHI,PHI_1],axis=1)
		Apred=self.inverse(x)
		LI=self.loss(A,Apred,sample_weight=self.gamma**np.arange(Apred.shape[0],0,-1))
		LF=self.loss(PHI_1,PHI_1_H,sample_weight=self.gamma**np.arange(PHI_1_H.shape[0],0,-1)).numpy()
