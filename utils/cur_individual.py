...
class Individual:
	...
	def __init__(self,env,genome=[],std=0.001):
		...
		self.nn.dense(NumpyLayer(input=32,unit=32, activation='relu',name="LR2"))
		...
		self.smooth_tanh=100.0
		self.w=100.0 if env.type=='maze' else 1.0
		if len(genome)>0:
	...
	def __call__(self, x):
		...
		# action=np.random.normal(action,np.ones(shape=action.shape)*1)
