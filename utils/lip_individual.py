import os
import sys
from utils.evonump import NumpyLayer
from utils.evonump import LipNeuralNetNumpy as NeuralNetNumpy
import ray 
import json
import numpy as np
...
class Individual:
	...
	def __init__(self,env,genome=[],std=0.001):
		...
		self.nn.dense(NumpyLayer(input=observation_shape[0],unit=16, activation='relu',name="LR1"))
		# self.nn.dense(NumpyLayer(input=16,unit=16, activation='relu',name="LR2"))
		self.nn.dense(NumpyLayer(input=16,unit=16, activation='relu',name="LR3"))
	...
	def eval(self,env):
		...
		while not done: 
			...
			# bcoord
			b=info['bcoord']
		return {"genome": self.nn.genome,"value": value,'s':s, 'bcoord':b, 'S': S, 'A': A }
...
if __name__=="__main__":
	...
	# env=Finger()
	# env=Stacker()
	env=CMaze(filename='SNAKE',time_horizon=1000, n_beams=8)
	i=Individual.remote(env)
	g=ray.get(i.genome.remote())
	i.set_genome.remote(g)
